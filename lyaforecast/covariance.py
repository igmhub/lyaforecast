...
class Covariance:
    ...
    def __call__(self, zmin, zmax):
        ...
        # self.lmin = self._cosmo.LYA_REST * (1 + self._zmin)
        # self.lmax = self._cosmo.LYA_REST * (1 + self._zmax)
        # self._z_mean = 0.5 * (self._zmin + self._zmax)
    ...
    def _get_zq_bin(self, lya_tracer):
        """
        Given wavelength range covered in bin,
        compute central tracer redshift and mean wavelength
        """
    ...
    def compute_eff_density_and_noise(self, corr_name):
        ...
        # These only matter if we have a tracer auto-correlation
        # tracer weights
    ...
    #     total_power_degkms = self._compute_total_power_lya(self._z_mean, kt_deg, kp_kms)
    ...
    #     # lya auto
    #     total_power_lya = self._compute_total_power_lya(z, kt_deg, kp_kms)
    #     total_power_lya_hmpc = total_power_lya * dhmpc_ddeg**2 / dkms_dhmpc

    #     # cross
    #     cross_tracer = 'lya_' + self._tracer_name
    #     cross = self._power_spec.compute_p3d_hmpc_smooth(z, k_hmpc, mu, self._pix_kms, self._res_kms, which=cross_tracer)

    #     # tracer auto
    #     tracer_auto = self._power_spec.compute_p3d_hmpc(z, k_hmpc, mu, which=self._tracer_name)
