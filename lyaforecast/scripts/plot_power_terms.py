...
if __name__ == '__main__':
    ...
    for j,cfg in enumerate(args.config):
        ...
        z_bin_edges = forecast._survey.z_bin_edges
        zc = forecast._survey.z_bin_centres[0]

        lmin = forecast._cosmo.LINE_REST * (1 + z_bin_edges[0,0])
        lmax = forecast._cosmo.LINE_REST * (1 + z_bin_edges[1,0])

        forecast._covariance(lmin, lmax)
        forecast._covariance.compute_eff_density_and_noise()

        p_n_eff = forecast._covariance._effective_noise_power[-1]
        ...
        for i,mu in enumerate(mus):
            ...
            kp_skm = kp_hmpc / forecast._cosmo.velocity_from_distance(zc)
            kt_deg = kt_hmpc * forecast._cosmo.distance_from_degrees(zc)

            p3d = forecast._power_spec.compute_p3d_kms(zc,kt_deg,kp_skm,
                                                        120,
                                                        60,forecast._absorption_line)
                        
            aliasing = forecast._covariance.compute_aliasing(zc,kt_deg,kp_skm)
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_ylabel(rf'Power $(k,\mu,z={zc})$')
